 ...
 */
let uploadJson = {}
let backImg1, templateId1, topologyId1
...
propertyPanel.prototype.createPanel = function (editor, vmvrId, name, selected, targetDeviceId, templateId, moduleId, uri, isShow) {
    ...
    a["ECVR"].push({
        "title": name,
        "url": uri + "?vmvrId=" + vmvrId + "&templateId=" + templateId + "&moduleId=" + moduleId + "&envTemplateId=" + editor.templateId + "&title=" + name + "&size=" + size,
        "targetNetwork": targetDeviceId
    });
};
...
networkTopologyEditor.prototype.initMenus = function () {
    ...
    self.nodeMainMenu.on("click", function (e) {
        ...
        if (text == "删除节点(Delete)") {
            ...
            let ajson = {}
            ajson = editor.stage.toJson();
            console.log(JSON.stringify((ajson)))
            ajson = decodeJson(ajson)
...
networkTopologyEditor.prototype.init = function (backImg, templateId, topologyId, stageJson, templateName) {
    ...
    //         //重建连线
    //         //中间小节点
    //         var m = new JTopo.Link(nodeA, nodeZ);
    //         m.lineType = "line";
    //         m.strokeColor = "0,255,255";
...
function getStageJson() {
    ...
    console.log(decodeJson(topologyJSON));
}

function getStageJson2() {
    topologyJSON = editor.stage.toJson();
    json = decodeJson(topologyJSON);
...
editor.utils = {
    ...
    //修改路径颜色
    reloadLink: function (json) {
        data = ["s1", "s2", "h1"]
        for (var i = 0; i < data.length - 1; i++) {
            var nodeA = editor.utils.findNode(data[i]);
            var nodeZ = editor.utils.findNode(data[i + 1]);
            //todo移除当前连线
    ...
    //跳转流表
    gotoft: function () {
        window.parent.parent.xadmin.add_tab('下发流表', 'member-list1.html')
    },
    //保存节点新的状态
...
function encodeJson(data) {
    ...
    var hnum = getJsonLength(data["hosts"]);
    var snum = getJsonLength(data["switches"]);
    var cnum = getJsonLength(data["controllers"]);
    var lnum = getJsonLength(data["links"]) - 1;
    json["topologyJson"].deviceNum = (hnum + snum + cnum).toString();
    var lnum = getJsonLength(data["links"]) - 1;
    var num = 0; //节点和连线的总数量
    //修改json格式

    for (var p in data["hosts"]) {
        node.x = ((hnum + snum + cnum) * 100 * Math.random()).toString();
        node.y = ((hnum + snum + cnum) * 100 * Math.random()).toString();
        var n = node;
        n["deviceId"] = p;
        n.text = p;
        n["dataType"] = "Py_VM";
        n["nodeImage"] = "Py_VM.png";
        var x = JSON.parse(JSON.stringify(n));
        json["topologyJson"]["childs"][0].childs[num++] = x;
    }
    for (var p in data["switches"]) {
        ...
        n["text"] = p;
        n["dataType"] = "Py_EC";
        n["nodeImage"] = "py_EC.png";
        var x = JSON.parse(JSON.stringify(n));
        json["topologyJson"]["childs"][0].childs[num++] = x;
    }
    for (var p in data["controllers"]) {
        node.x = ((hnum + snum + cnum) * 100 * Math.random()).toString();
        node.y = ((hnum + snum + cnum) * 100 * Math.random()).toString();
        var n = node;
        n["deviceId"] = p;
        n["text"] = p;
        n["dataType"] = "Py_VR";
        n["nodeImage"] = "servervm.png";
        var x = JSON.parse(JSON.stringify(n));
        json["topologyJson"]["childs"][0].childs[num++] = x;
    }
    for (var i = 0; i < lnum; i++) {
        var l = link;
        var a = data["links"][i][0];
        var b = data["links"][i][1];
        l["deviceA"] = a > b ? a : b;
        l["deviceZ"] = a <= b ? a : b;
        var l = JSON.parse(JSON.stringify(l));
        json["topologyJson"]["childs"][0].childs[num++] = l;
    }
    for (var p in data["controllers"]) {
        for (var i=0;i<data.controllers[p].length;i++) {
            var l = link;
            l["deviceA"] = p;
            l["deviceZ"] = data.controllers[p][i];
            var xl = JSON.parse(JSON.stringify(l));
            json["topologyJson"]["childs"][0].childs[num++] = xl;
        }
    }
...
function decodeJson(data) {
    ...
    var numc = 0
    var carray = []
    for (var p in data.childs[0].childs) {
        var str = data.childs[0].childs[p].text;
        var obj = data.childs[0].childs[p];


        if (obj.elementType == undefined) continue;
        else if (obj.elementType == "link") {
            if (obj.deviceA[0] == 'c' || obj.deviceZ[0] == 'c') {
                if (obj.deviceA[0] == 'c') {
                    if (carray.indexOf(obj.deviceA) == -1) {
                        carray.push(obj.deviceA)
                        json.controllers[obj.deviceA] = [];
                    }
                    json.controllers[obj.deviceA].push(obj.deviceZ)

                } else {
                    if (carray.indexOf(obj.deviceZ) == -1) {
                        carray[carray.length] = obj.deviceZ
                        json.controllers[obj.deviceZ] = [];
                    }
                    json.controllers[obj.deviceZ].push(obj.deviceA)
                }
            } else {
                var arr = []
                arr.push(obj.deviceA, obj.deviceZ)
                json.links.push(arr);
            }
...
function fileImport() {
    ...
    reader.onload = function () {
        ...
        uploadJson = encodeJson(uploadJson);

        backImg1 = "images/backimg.png";
        let topologyJson = uploadJson.topologyJson;
        // window.location.reload();
        editor.deleteAllNodes(editor.templateId)
        sleep(3000)
        // editor.init(backImg1, templateId1, topologyId1, topologyJson, "");
...
function showItem() {
    appendtext("ICMP:id&nbsp1string&nbspcreate&nbspsuccessful......")
...
function loadChart1() {
    appendtext("run&nbsp;the&nbsp;test:ete_throughput......")
    setTimeout("xxxx1()", 3000)
}

function xxxx1() {
    window.parent.parent.xadmin.add_tab("最大容量测试", 'echarts' + '1' + '.html');

}

function loadChart2() {
    appendtext("run&nbsp;the&nbsp;test:ete_latency......")
    setTimeout("xxxx2()", 3000)
}

function xxxx2() {
    window.parent.parent.xadmin.add_tab("时延测试", 'echarts' + '2' + '.html');

}

function loadChart3() {
    appendtext("run&nbsp;the&nbsp;test:controller_chain......")
    setTimeout("xxxx3()", 3000)
}

function xxxx3() {
    window.parent.parent.xadmin.add_tab("控制信道容量", 'echarts' + '3' + '.html');


}

function loadChart4() {
    setTimeout("xxxx4()", 3000)
}

function xxxx4() {
    appendtext("run&nbsp;the&nbsp;test:flow_query_time......")
    window.parent.parent.xadmin.add_tab("流表查询时间", 'echarts' + '4' + '.html');

}

function loadChart5() {
    setTimeout("xxxx5()", 3000)
}

function xxxx5() {
    appendtext("run&nbsp;the&nbsp;test:flow_time......")
    window.parent.parent.xadmin.add_tab("流表收敛时间", 'echarts' + '5' + '.html')
...
json=JSON.stringify((json))
    $.ajax({
        type: 'post',
        url: "http://192.168.123.123:5000/net/run/0", //上传文件的请求路径必须是绝对路劲
        dataType:"json",
        data:json
...
function stopnet(){
    $.ajax({
        type: 'post',
        url: "http://192.168.123.123:5000/net/stop",
    }).success(function (data) {
        console.log("success")
        alert("网络已停止")
        appendtext("网络已停止")
...
function appendtext(string){
    ...
    var div=dom.firstChild;
    var p=document.createElement("p")
    p.innerHTML=string;
    sleep(1000)
    dom.insertBefore(p,div);
}
